...
export class AStar {
    ...
    findInOpenQueue = (nodeId: string): number => {
        if (this.searchableOpenQueue === undefined) throw new Error('must convert queue to array before searching for node');
        return this.searchableOpenQueue.findIndex((node: PathNode) => node.id === nodeId);
    }
    selectCheaperOpenNode = (child: PathNode, index: number) => {
        if (this.searchableOpenQueue === undefined) throw new Error('must convert queue to array before replacing for node');
        const prev = this.searchableOpenQueue[index];
        if (prev.cost > child.cost) {
            this.opened = MinPriorityQueue.fromArray(this.searchableOpenQueue.filter((node: PathNode) => node.id !== prev.id))
